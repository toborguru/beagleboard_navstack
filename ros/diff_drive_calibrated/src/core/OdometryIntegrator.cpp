 ...
 */
void OdometryIntegrator::onEncoderCountsAvailableEvent( diff_drive_calibrated::EncoderCounts const & encoder_counts )
{
  const diff_drive_calibrated::EncoderCounts* p_new_message;

  p_new_message = new diff_drive_calibrated::EncoderCounts( encoder_counts );
 ...
 *
 */
void OdometryIntegrator::addNewCounts( diff_drive_calibrated::EncoderCounts const & counts )
{
  BaseVelocities_T  velocities;
  nav_msgs::Odometry new_position;
  nav_msgs::Odometry old_position;
  diff_drive_calibrated::MovementStatus new_movement_status;
 ...
 *
 */
diff_drive_calibrated::MovementStatus OdometryIntegrator::calculateMovementStatus( BaseVelocities_T const & velocities,
                                                                                BaseModel const & base_model )
{
  ...
  diff_drive_calibrated::MovementStatus movement_status;

  if ( base_model.getSetupValid() == false )
  {
    movement_status.motors_state = diff_drive_calibrated::MovementStatus::SETUP_ERROR;
  ...
  else
  {
    ...
    {
      movement_status.motors_state = diff_drive_calibrated::MovementStatus::CORRECT;
      movement_status.stasis_wheel_enabled = false;
    }
    else // Stasis Wheel Active
    {
      ...
      if ( (abs_linear > _velocity_lower_limit) || (abs_stasis > _velocity_lower_limit) )
      {
        ...
        {
          movement_status.motors_state = diff_drive_calibrated::MovementStatus::FREE_WHEELING;
        }
        else if (abs_stasis < lower_limit)
        {
          movement_status.motors_state = diff_drive_calibrated::MovementStatus::STASIS;
        }
        else
        {
          movement_status.motors_state = diff_drive_calibrated::MovementStatus::CORRECT;
                                              ...
                                              diff_drive_calibrated::MovementStatus const & movement_status ) const
{
  ...
  if ( movement_status.motors_state == diff_drive_calibrated::MovementStatus::CORRECT )
  {
 ...
 */  
void OdometryIntegrator::notifyMovementStatusListeners( diff_drive_calibrated::MovementStatus const & movement_status) const
{
  pthread_mutex_lock( _p_data_mutex );
...
{
  const diff_drive_calibrated::EncoderCounts* p_message;
